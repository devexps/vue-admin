...
import { omit, cloneDeep } from '@devexps/utils'
...
function dynamicImport(
  ...
  if (matchKeys?.length === 1) {
    ...
    console.warn(
      'could not find under /src/pages/' +
        component +
        '.vue or ' +
        component +
...
export function transformObjToRoute<T = RouteRecordItem>(
  ...
  routeList.forEach((route) => {
    ...
    if (component) {
      ...
      console.warn('Please configure routing correctly: ' + _route?.name + ' the component attribute')
    }
