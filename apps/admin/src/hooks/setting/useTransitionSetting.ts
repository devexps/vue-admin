import type { TransitionSetting } from '@devexps/types'
...
export function useTransitionSetting() {
  ...
  const getEnableTransition = computed(
    () => configStore.getTransitionSetting?.enable,
  )

  const getOpenNProgress = computed(
    () => configStore.getTransitionSetting?.openNProgress,
  )
  ...
  const getBasicTransition = computed(
    () => configStore.getTransitionSetting?.basicTransition,
  )
